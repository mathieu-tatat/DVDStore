...
public class DvdStoreService {
    ...
    public ArrayList<DvdServiceModel> getAll() {
        ...
        for (DvdRepositoryModel x : dvdRepositoryModelsArrayList) {
            dvdServiceModels.add(new DvdServiceModel(x.getName(), x.getGenre(),x.getQuantity(), x.getPrix(), x.getPhoto(), x.getDescription(),Optional.ofNullable(x.getId())));
        }
    ...
    public DvdServiceModel getDvdById(Long id) {
        DvdRepositoryModel dvdRepositoryModel = dvdRepositoryInterface.findById(id).get();
        return new DvdServiceModel(dvdRepositoryModel.getName(), dvdRepositoryModel.getGenre(),dvdRepositoryModel.getQuantity(),dvdRepositoryModel.getPrix(),dvdRepositoryModel.getPhoto(),dvdRepositoryModel.getDescription(), Optional.ofNullable(dvdRepositoryModel.getId()));
    }
